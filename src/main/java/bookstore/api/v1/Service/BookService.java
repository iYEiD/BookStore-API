...
public class BookService {
    ...
    public Book addBook(Book book) {
        if (bookRepository.findById(book.getId()).isPresent()) {
            System.out.println("Book already exists (Or ID already taken), The book was not added");
            return null;
        }
        if (book.getTitle().isEmpty() || book.getAuthor().isEmpty() || book.getPrice() <= 0 || book.getStock() < 0)
            return null;
